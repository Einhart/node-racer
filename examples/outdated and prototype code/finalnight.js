...
var highThresh = 100;
var nIters = 2;
var minArea = 866; // decides minimum size of square. try higher amount for closer square. too small makes squares out of pixels
...
setInterval(function() {
      ...
      if (err) {
        console.log(err);
         ...
         var out = new cv.Matrix(height, width);
          ...
          //imm.inRange([0, 0, 138], [186, 138, 255]);// this function will color white what it deems as red.
          imm.inRange([168,80,80], [190,255,255]);//check red hsv
          im_canny = imm.copy();
            im_canny.erode(8);
            im_canny.dilate(8);
  ...
  for (var i = 0; i < contours.size(); i++) {
    ...
    contours.approxPolyDP(i, 0.138 * arcLength, true); // original 0.138
...
//if ((solidity > 0.9) && (aspectRatio>=0.6 && aspectRatio<=1.6)){
if ((solidity > 0.9) && (aspectRatio>=1.2 && aspectRatio<=1.8)){
    switch(contours.cornerCount(i)) {
      case 4:
        //im.drawContour(contours,i, RED, 6);
        im.rectangle([boundingRect.x,boundingRect.y],[boundingRect.width,boundingRect.height],RED,6); // after contours are detected to be 4, draw a rec around the centre
...
drone.stop();
      drone.postureJumper();
      drone.forward(38);
      setTimeout(function() {drone.stop();}, 2000);
//}, 1000);
...
if(centerX <= 288){//if less than or equal to x=300
          drone.left(8); // wheels tend to be sticky at slow speeds.
      setTimeout(function() {
        drone.stop();
      }, 1);
                }
else if(centerX > 352){// if greater than or equal to x=360
          drone.right(8);
      setTimeout(function() {
        drone.stop();
      }, 1);
     ...
     // }, 1);
      ...
      closing(bx,by,bwidth,bheight);
        }
}

function closing(bx,by,bwidth,bheight){
    if(bwidth<80){
        drone.forward(30); //for forward motion 10 is sufficient. it is more robust than backwards
      setTimeout(function() {
      drone.stop();}, 1);
    }
    else if (bwidth<320){
      ...
      setTimeout(function() {drone.stop();}, 1);
    }
    else{
