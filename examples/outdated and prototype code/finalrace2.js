...
var highThresh = 100;
var nIters = 2;
var minArea = 688; // decides minimum size of square. try higher amount for closer square. too small makes squares out of pixels
      ...
      if (err) {
        console.log(err);
         ...
         var out = new cv.Matrix(height, width);
          ...
          im_canny.gaussianBlur([9,9]); //gaussian blur made everything stable
...
if ((solidity > 0.9) && (aspectRatio>=0.6 && aspectRatio<=1.2)){
...
if(centerX <= 280){//if less than or equal to x=300
...
function closing(bx,by,bwidth,bheight){
    if(bwidth<60){
      ...
      drone.stop();}, 1);
            ...
            drone.forward(8); //for forward motion 10 is sufficient. it is more robust than backwards
      setTimeout(function() {
      drone.stop();}, 1);
    }
